...
from .PromptsPlan import PromptsPlan


class CSV2PromptsPlan(PromptsPlan):
    """
    A class for creating CSV2PromptsPlan objects from CSV data.

    Example Usage:
        plan = CSV2PromptsPlan.from_csv_row(csv_row)
        plans = CSV2PromptsPlan.from_csv_file(csv_file_path)

    """
    @classmethod
    def from_csv_row(cls, row: Dict[str, str]) -> 'CSV2PromptsPlan':
        """Create a CSV2PromptsPlan object from a CSV row."""
        # Convert CSV row data types to match PromptsPlan's expected types
        data = {
    ...
    @classmethod
    def from_csv_file(cls, csv_file_path: str) -> Dict[str, List['CSV2PromptsPlan']]:
        """Create CSV2PromptsPlan objects from a CSV file."""
        plans = {}
...
def create_prompt_plan_files(csv_file_path: str, output_dir: str):
    """Create PromptsPlan JSON files from a CSV file."""
    plans = CSV2PromptsPlan.from_csv_file(csv_file_path)
    ...
    print(f"Created {len(plans)} PromptsPlan JSON file(s) in {output_dir}")


def main():
    parser = argparse.ArgumentParser(description="Create PromptsPlan JSON files from a CSV file.")
