...
class Codex2Plan2Codex(Codexes2Parts):
    ...
    def run_condenser_prompts(self, condenser_prompts):
        ...
        try:
            ...
            self.logger.error(f"Problematic string: {condenser_prompts_string}")

            # Attempt to rescue the string
            try:
                # Find the last occurrence of a closing brace
                last_brace_index = condenser_prompts_string.rfind("}")

                if last_brace_index != -1:
                    # Truncate the string to include only the valid JSON
                    condenser_prompts_string = condenser_prompts_string[:last_brace_index + 1]
                    condenser_prompts_dict = json.loads(condenser_prompts_string)
                    self.logger.warning(f"Rescued JSON by truncating extra data.")
                else:
                    self.logger.error("Could not rescue JSON. Aborting condenser sequence.")
                    return None  # Or handle the error appropriately
            except json.JSONDecodeError as e:
                self.logger.error(f"Failed to rescue JSON: {e}")
                return None  # Or handle the error appropriately
...
class Codex2DynamicPlans2Codex(Codexes2Parts):
    def run_dynamic_plan(self, plan: PromptsPlan) -> List[str]:
        # Initialize Codexes2PartsOfTheBook
        codex_processor = Codexes2Parts()
    ...
    def create_dynamicPlan(self, filepath, context):
        condenserPlan = PromptsPlan(
            name="condenserPlan",
            textfilename=filepath,
            require_json_output=True,
    ...
    def run_dynamic_prompts(self, condenser_prompts):
        # extract Plan vaues from condenser_prompts
        print(type(condenser_prompts))
        condenser_prompts_string = condenser_prompts[0]
        condenser_prompts_string = condenser_prompts_string.replace("```json", "").replace("```", "")
