...
class PartsBuilder:
    ...
    - use_continuation_prompt(self, plan: PromptsPlan, initial_content: str) -> str
    - build_part(self, plan: PromptsPlan) -> str
    - build_multi_part(self, plan: PromptsPlan) -> str
    - build_parts_from_codex(self, codex: str, plans: List[PromptsPlan]) -> List[str]
    """
    def __init__(self):
    ...
    def build_part(self, plan: PromptsPlan) -> str:
        """Build a single part based on the given PromptsPlan."""

        return self.c2p.process_codex_to_book_part(plan)

    def build_multi_part(self, plan: PromptsPlan) -> str:
        """Build multiple parts within a single PromptsPlan."""
    ...
    def build_parts_from_codex(self, codex: str, plans: List[PromptsPlan]) -> List[str]:
