...
def batchified_chatcomplete(text, reader):
    reader.pagetext = {"role": "system", "content": pagetext}
    prompts = [reader.system_yourjob]
    prompts.append(reader.demographic_attributes)
    prompts.append(reader.pagetext)

    print('prompts:')
    print(prompts)
    assignmentsArray = reader.on_page_assignments
    stringifiedAssignmentsArray = json.dumps(assignmentsArray)
    print('stringifiedAssignmentsArray:')
    print(stringifiedAssignmentsArray)
    prompts.append({"role": "user", "content": stringifiedAssignmentsArray})
    batchInstruction = {
        "role":
            "system",
        "content":
    ...
    result = openai.ChatCompletion.create(model="gpt-3.5-turbo", messages=prompts, max_tokens=1000)
    print(result)
    batchCompletion = json.loads(result.choices[0].message.content)
...
if __name__ == "__main__":
    ...
    '''
    r = Reader()
    r.current_page_text = pagetext
    result = batchified_chatcomplete(pagetext, r)
    print(result)
